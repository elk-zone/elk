...
export class WebSocketSubscription implements akkoma.streaming.Subscription {
  ...
  async *values(): AsyncIterableIterator<akkoma.streaming.Event> {
    try {
  ...
  [Symbol.asyncIterator](): AsyncIterableIterator<akkoma.streaming.Event> {
    return this.values();
  }
  ...
  private test(event: akkoma.streaming.Event): boolean {
  ...
  private parseMessage(rawEvent: string): akkoma.streaming.Event {
    const data = this.serializer.deserialize<akkoma.streaming.RawEvent>(
      "json",
      rawEvent,
    );
