      ...
      <div flex flex-col gap-3>
        <div v-if="reblogs.length" flex="~ gap-1">
          ...
          <div ml1>
            ...
            <time text-secondary :datetime="reblogsTimeAgoCreatedAt"> ・{{ reblogsTimeAgo }} </time>
          </div>
        </div>
        <div v-if="likes.length" flex="~ gap-1 wrap">
          <div
            :class="
              useStarFavoriteIcon ? 'i-ri:star-line color-yellow' : 'i-ri:heart-line color-red'
            "
            text-xl
            me-2
          />
          ...
          <div ms-4>
            ...
            <time text-secondary :datetime="likesTimeAgoCreatedAt"> ・{{ likesTimeAgo }} </time>
          </div>
        </div>
      </div>
      <div ps9 mt-1>
        ...
        <template v-if="!group.status!.content">
          <StatusMedia
            ...
            pointer-events-none
          />
          <StatusPoll v-else-if="group.status!.poll" :status="group.status!" />
        </template>
      </div>
    </StatusLink>
  </article>
