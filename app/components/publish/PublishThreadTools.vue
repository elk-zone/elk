...
const isRemovableItem = computed(
  () => threadIsActive.value && draftItemIndex < threadItems.value.length - 1,
)
...
<template>
  <div flex flex-row rounded-3 :class="{ 'bg-border': threadIsActive }">
    <div
      ...
      gap="0.5"
    >
      {{ draftItemIndex + 1 }}<span text-secondary-light>/</span
      ><span text-secondary-light>{{ threadItems.length }}</span>
    </div>
