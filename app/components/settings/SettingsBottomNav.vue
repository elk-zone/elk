...
const navButtonNamesSetting = useLocalStorage<NavButtonName[]>(
  STORAGE_KEY_BOTTOM_NAV_BUTTONS,
  defaultSelectedNavButtonNames.value,
)
const selectedNavButtonNames = ref<NavButtonName[]>(navButtonNamesSetting.value)

const selectedNavButtons = computed<NavButton[]>(() =>
  selectedNavButtonNames.value.map(
    (name) => availableNavButtons.find((navButton) => navButton.name === name)!,
  ),
)
...
function remove(navButtonName: NavButtonName) {
  selectedNavButtonNames.value = selectedNavButtonNames.value.filter(
    (name) => name !== navButtonName,
  )
}
  ...
  <section space-y-2>
    ...
    >
      ...
      <div aria-hidden="true" flex="~ gap4 wrap" items-center select-settings h-14>
        <nav
          v-for="availableNavButton in selectedNavButtons"
      ...
      <div flex="~ col" gap-y-4 gap-x-2 py-1 sm="~ justify-end flex-row">
        ...
        <button btn-solid font-bold py2 full-w sm-wa flex="~ gap2 center" :disabled="!canSave">
