...
const meta = computed(() => {
  ...
  if (
    !firstName ||
    (reservedNames.check(firstName) && !supportedReservedRoutes.includes(firstName))
  )
    return undefined
        ...
        <div flex flex-col gap-2>
          ...
          >
            <template v-if="meta.repo">
              <span>{{ meta.user }}</span
              ><span text-secondary-light>/</span
              ><span text-primary font-bold>{{ meta.repo }}</span>
            </template>
            <span v-else>{{ meta.user }}</span>
          </NuxtLink>
          <NuxtLink sm:text-lg :href="card.url" target="_blank" external>
            <span v-if="meta.type === 'issue'" text-secondary-light me-2> #{{ meta.number }} </span>
