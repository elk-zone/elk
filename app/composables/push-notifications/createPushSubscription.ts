...
export async function createPushSubscription(
  ...
  return await getRegistration()
    ...
    .then(
      ({ registration, subscription }): Promise<mastodon.v1.WebPushSubscription | undefined> => {
        if (subscription) {
          ...
          if (
            ...
            return unsubscribeFromBackend(false, false)
              ...
              .then((subscription) =>
                sendSubscriptionToBackend(subscription, notificationData, policy),
              )
          }
        }

        return subscribe(registration, vapidKey).then((subscription) =>
          sendSubscriptionToBackend(subscription, notificationData, policy),
        )
      },
    )
    .catch((error) => {
      ...
      if (error.code === 11 && error.name === 'InvalidStateError')
        ...
        useError = new PushSubscriptionError(
          'vapid_not_supported',
        ...
        useError = new PushSubscriptionError(
          'invalid_vapid_key',
          `The VAPID public key seems to be invalid: ${vapidKey}`,
        )
...
): Promise<PushManagerSubscriptionInfo> {
