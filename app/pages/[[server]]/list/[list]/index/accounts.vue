...
const accountsInList = ref(
  await useMastoClient().v1.lists.$select(listId.value).accounts.list({ limit: 1000 }),
)
...
<template>
  ...
  <div ref="el" relative group>
    <form border="t base" p-4 w-full flex="~ wrap" relative gap-3>
      <div
        bg-base
        border="~ base"
        flex-1
        ...
        <template v-else-if="!loading">
          <template v-if="results.length > 0">
            ...
            >
              ...
              <CommonTooltip
                :content="
                  isInCurrentList(result.id) ? $t('list.remove_account') : $t('list.add_account')
                "
              >
                <button
                  text-sm
                  ...
                  @click="
                    () =>
                      isInCurrentList(result.id)
                        ? removeAccount(result.data)
                        : addAccount(result.data)
                  "
                >
                  <span
                    :class="
                      isInCurrentList(result.id) ? 'i-ri:user-unfollow-line' : 'i-ri:user-add-line'
                    "
                  />
                </button>
              </CommonTooltip>
  ...
  <CommonPaginator ref="paginatorRef" :paginator="paginator">
    <template #default="{ item }">
      <ListAccount :account="item" :list="listId" hover-card border="b base" py2 px4 />
    </template>
  </CommonPaginator>
</template>
