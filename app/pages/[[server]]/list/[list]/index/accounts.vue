...
const mastoListAccounts = useAkkoClient().v1.lists.$select(listId.value).accounts
...
const accountsInList = ref((await useAkkoClient().v1.lists.$select(listId.value).accounts.list({ limit: 1000 })))
...
function addAccount(account: akkoma.v1.Account) {
  try {
...
function removeAccount(account: akkoma.v1.Account) {
  try {
