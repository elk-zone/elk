...
<template>
  <div my-8 px-3 sm:px-8 md:max-w-200 flex="~ col gap-4" relative>
    ...
    <img :alt="$t('app_logo')" :src="`${useRuntimeConfig().app.baseURL || ''}/logo.svg`" w-20 h-20 height="80" width="80" mxa class="rtl-flip">
    <h1 mxa text-4xl mb4>
      {{ $t('help.title') }}
    </h1>
    <p>
    ...
    <p flex="~ gap-2 wrap" mxa>
      <template v-for="team of elkTeamMembers" :key="team.github">
        <NuxtLink :href="`https://github.com/sponsors/${team.github}`" target="_blank" external rounded-full transition duration-300 border="~ transparent" hover="scale-105 border-primary">
          <img :src="`${useRuntimeConfig().app.baseURL || ''}/avatars/${team.github}-100x100.png`" :alt="team.display" rounded-full w-15 h-15 height="60" width="60">
        </NuxtLink>
      </template>
    </p>
