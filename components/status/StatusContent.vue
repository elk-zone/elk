...
<template>
  ...
  >
    <StatusBody v-if="!isFiltered && status.sensitive && !status.spoilerText" :status="status" :newer="newer" :class="isDetails ? 'text-xl' : ''" />
    <StatusSpoiler :enabled="status.sensitive || isFiltered" :filter="isFiltered" :is-d-m="isDM">
      ...
      <StatusBody v-if="!status.sensitive || status.spoilerText" :status="status" :newer="newer" :class="isDetails ? 'text-xl' : ''" />
