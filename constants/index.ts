...
export const STORAGE_KEY_CLIENT_APP = 'elk-client-app'

export const HANDLED_MASTO_URLS = /^(https?:\/\/)?([\w\-]+\.)+\w+\/(@[@\w\-.]+)(\/objects)?(\/\d+)?$/

export const NOTIFICATION_FILTER_TYPES: akkoma.v1.NotificationType[] = ['mention', 'reblog', 'follow', 'follow_request', 'favourite', 'poll', 'pleroma:report'] // not supported for now 'update', 'admin.sign_up', 'admin.report'
