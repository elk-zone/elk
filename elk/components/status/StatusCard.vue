    ...
    <div flex gap-3 :class="{ 'text-secondary': inNotification }">
      ...
      <template v-else>
        ...
        <div flex="~ col 1" min-w-0>
          ...
          />
          <StatusActions v-if="actions !== false" v-show="!getPreferences(userSettings, 'zenMode')" :status="actionStatus" :can-reblog="canReblog" :is-loading="isLoading" :toggle-bookmark="toggleBookmark" :toggle-react="toggleReact" :toggle-reblog="toggleReblog" />
        </div>
      </template>
    </div>
  </StatusLink>
