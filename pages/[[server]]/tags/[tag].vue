...
const { data: tag, refresh } = $(await useAsyncData(() => client.v1.tags.$select(tagName.value).fetch(), { default: () => shallowRef() }))

const paginator = client.v1.timelines.tag.$select(tagName.value).list()
