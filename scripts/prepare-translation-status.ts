...
export const localeData: [code: string, file: string[], title: string][] = currentLocales.map(
  (l: any) => [l.code, l.files ? l.files : [l.file!], l.name ?? l.code],
);
...
async function readI18nFile(file: string | string[]) {
  ...
  if (Array.isArray(file)) {
    const files = await Promise.all(
      file.map((f) => async () => {
        return JSON.parse(
          Buffer.from(await readFile(resolver.resolve(`../locales/${f}`), "utf-8")).toString(),
        );
      }),
    ).then((f) => f.map((f) => f()));
    const data: Record<string, any> = files[0];
    void files.splice(0, 1);
     files.forEach((f) => merge(f, data));
    return data;
  } else {
    return JSON.parse(
      Buffer.from(await readFile(resolver.resolve(`../locales/${file}`), "utf-8")).toString(),
    );
  }
}
